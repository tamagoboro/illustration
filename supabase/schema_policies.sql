...
create policy "Owners can update their own forms" on public.estimate_forms as PERMISSIVE for UPDATE to public
  using ((auth.uid() = user_id))
  with check ((auth.uid() = user_id));
create policy "Owners manage own portfolio items" on public.portfolio_items as PERMISSIVE for ALL to public
  using ((auth.uid() = user_id))
  with check ((auth.uid() = user_id));
...
create policy "Public can view estimate forms" on public.estimate_forms as PERMISSIVE for SELECT to public
  using (true);
create policy "Public can view portfolio items" on public.portfolio_items as PERMISSIVE for SELECT to public
  using (true);
...
create policy "admins can view reports" on public.reports as PERMISSIVE for SELECT to authenticated
  using ((EXISTS ( SELECT 1
   FROM admins
  WHERE (admins.user_id = auth.uid()))));
create policy "analytics_logs limited insert" on public.analytics_logs as PERMISSIVE for INSERT to anon, authenticated
  with check (((creator_id IS NOT NULL) AND (event_type = ANY (ARRAY['pv'::text, 'estimate_calc'::text, 'favorite'::text])) AND (octet_length((COALESCE(metadata, '{}'::jsonb))::text) <= 2000)));
...
CREATE UNIQUE INDEX point_transactions_starter_bonus_uniq ON public.point_transactions USING btree (user_id) WHERE (reason = 'starter_bonus'::text);
