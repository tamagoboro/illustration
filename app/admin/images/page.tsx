...
export default function AdminImagesPage() {
  ...
  const loadTarget = async (user: FoundUser) => {
    setTarget(user)
  ...
  const searchUsers = async (q: string, autoOpen = false) => {
    ...
    if (data.length === 1 || autoOpen) {
      await loadTarget(data[0] as FoundUser)
    } else {
  ...
  const reloadCurrent = async () => {
    ...
    const { data: profile } = await supabase
      ...
      .maybeSingle()
    await loadTarget((profile as FoundUser) || target)
    await refreshAudit()
  }
  ...
  const replaceWork = async (item: WorkRow, field: 'image' | 'before', newUrl: string | null, uploadedPath?: string) => {
    const r = requireReason()
    if (!r) {
      if (uploadedPath) await removeStorageFiles([`/storage/v1/object/public/portfolios/${uploadedPath}`])
      return
    }
    setBusyKey(`work:${item.id}`)
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-2">
          ...
          >
            ...
            <button
              type="submit"
              disabled={searching}
              className={`${btnBase} bg-sky-500 hover:bg-sky-600 text-white px-4`}
            >
              {searching ? '検索中...' : '検索'}
            </button>
          </form>
        ...
        {target && (
          <>
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-14 h-14 rounded-full overflow-hidden bg-slate-100 shrink-0">
                  {target.avatar_url && <img src={target.avatar_url} alt="" className="w-full h-full object-cover" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-black text-slate-900 truncate">{target.display_name || '（名前なし）'}</p>
                  <Link href={`/creator/${target.user_id}`} target="_blank" className="text-[11px] font-bold text-sky-600 hover:underline">
                    公開ページを見る →
                  </Link>
                </div>
                {target.avatar_url && (
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  maxLength={200}
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl outline-none focus:border-sky-400"
                />
                <div className="flex flex-wrap gap-1.5">
                  {REASON_PRESETS.map((p) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setReason(p)}
                      className="text-[10px] font-bold bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-500 px-2 py-1 rounded-lg cursor-pointer"
                    >
                      {p}
                    </button>
                  ))}
            ...
            {loadingWorks ? (
              <p className="text-xs text-slate-600 font-bold text-center py-6">読み込み中...</p>
            ) : works.length === 0 ? (
              <p className="text-xs text-slate-600 font-bold text-center py-6">このクリエイターの作品はありません</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {works.map((w) => {
                  const busy = busyKey === `work:${w.id}`
                  return (
                    <div key={w.id} className="bg-white rounded-2xl p-3 shadow-sm border border-slate-100 space-y-2">
                      <div className="flex gap-2">
                        <div className="flex-1 aspect-square rounded-xl overflow-hidden bg-slate-100">
