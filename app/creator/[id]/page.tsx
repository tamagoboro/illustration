...
export default function CreatorClient({ id }: { id: string }) {
  ...
  // ※ Discord直リンクは切れるため、public/images/bg-note.png や Supabase Storage のURLを推奨します
  const BACKGROUND_IMAGE_URL = "https://qcklfkslqtjnxufqcqyi.supabase.co/storage/v1/object/public/portfolios/bg.png"
    ...
    >
      ...
      <header className="px-6 py-4 bg-white/70 backdrop-blur-xl border-b border-white/50 sticky top-0 z-30 shadow-xs">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <span className="text-[11px] font-black tracking-widest text-slate-400 uppercase">Creator Portfolio</span>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={i} className="p-3.5 bg-white/60 rounded-2xl border border-white/80 space-y-1 shadow-2xs">
                ...
                <span className={`text-xs font-extrabold block ${spec.highlight ? 'text-indigo-600' : 'text-slate-800'}`}>
                  {spec.value}
                </span>
              </div>
            ))}
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-baseline px-1">
            <h2 className="text-base font-black text-slate-900 tracking-tight drop-shadow-xs">ポートフォリオ</h2>
