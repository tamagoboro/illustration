...
export default function FormBuilderPage() {
  ...
  const [config, setConfig] = useState<FormConfig>({
    ...
    thanks_message: 'ご依頼ありがとうございます！2日以内にご連絡いたします。',
    theme_color: '#ec4899',
    is_accepting: true,
  ...
  const moveField = (idx: number, direction: 'up' | 'down') => {
    ...
    if (targetIdx < 0 || targetIdx >= config.fields.length) return
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20 font-sans">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 mb-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <button
            ...
            className="px-6 py-2.5 rounded-xl text-white font-black text-xs shadow-md hover:opacity-90 transition active:scale-95 disabled:opacity-50 cursor-pointer"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-2 bg-amber-400 hover:bg-amber-500 text-slate-800 font-black text-xs rounded-full shadow-xs cursor-pointer transition"
                >
                  ✨ テンプレート
                </button>
                <button
                  onClick={addField}
                  className="px-4 py-2 bg-pink-500 hover:bg-pink-600 text-white font-black text-xs rounded-full shadow-xs cursor-pointer transition"
                >
                  + 項目追加
                </button>
              </div>
              ...
              <div key={f.id} className="bg-white rounded-2xl p-5 border-2 border-slate-100 space-y-3 shadow-xs">
                <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 bg-slate-100 p-0.5 rounded-lg">
                      <button
                        ...
                        disabled={idx === 0}
                        title="上に移動"
                        className="p-1 text-[10px] font-black text-slate-600 hover:bg-white rounded disabled:opacity-30 disabled:hover:bg-transparent cursor-pointer transition"
                      >
                        ▲
                      </button>
                      <button
                  ...
                  <button
                    ...
                    className="text-xs font-bold text-red-400 hover:text-red-600 cursor-pointer transition"
                  >
                    削除
                  </button>
                </div>
        ...
        {/* ================= 右側：リアルタイムスマホ風プレビュー（スクロール・高さを最適化） ================= */}
        <div className="w-full lg:w-5/12 sticky top-20 h-[calc(100vh-100px)] flex flex-col space-y-2">
          {/* プレビュータブ切り替え */}
          <div className="flex bg-slate-200/80 p-1 rounded-2xl font-black text-xs shrink-0">
            <button
          ...
          {/* スマホ本体枠 */}
          <div className="bg-white rounded-[40px] shadow-2xl border-8 border-slate-800 p-5 overflow-hidden flex flex-col flex-1 min-h-0">
            {previewTab === 'input' ? (
              <div className="flex flex-col justify-between h-full overflow-y-auto pr-1">
                ...
                {/* 概算見積計算バー */}
                <div
                  className="mt-6 pt-4 border-t-2 border-slate-100 flex justify-between items-center p-4 rounded-2xl shrink-0"
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-2xl space-y-4 max-w-lg w-full border-4 border-amber-100">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setShowTmplModal(false)} className="text-slate-400 font-bold hover:text-slate-600 cursor-pointer">✕</button>
            </div>
