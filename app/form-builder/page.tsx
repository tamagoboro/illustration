...
export default function CreatorClient({
  ...
  const themeColor = activeFormConfig?.theme_color || '#ec4899'
  ...
  const totalPrice = useMemo(() => {
      ...
      if (field.type === 'radio') {
        ...
        if (selectedOpt) {
          if (selectedOpt.priceType === 'percent') {
            ...
            extraFixedSum += selectedOpt.price
          }
        }
    ...
    const totalBase = baseSum + extraFixedSum
    return totalBase + Math.round(totalBase * (percentSum / 100))
  ...
  const handleGenerateSpec = () => {
    ...
    activeFormConfig.fields.forEach((field) => {
      ...
      const answer = formAnswers[field.id]
      const label = field.label || '無題の項目'

      if (!answer || (Array.isArray(answer) && answer.length === 0)) {
        specLines.push(`■ ${label}: 未指定`)
        return
      }

      if (Array.isArray(answer)) {
        specLines.push(`■ ${label}: ${answer.join(', ')}`)
      } else {
        specLines.push(`■ ${label}: ${answer}`)
      }
    })
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      activeFormConfig.fields.forEach((field) => {
        ...
        if (!answer || (Array.isArray(answer) && answer.length === 0)) {
          formattedAnswers.push({ label, value: '未指定' })
        } else if (Array.isArray(answer)) {
          formattedAnswers.push({ label, value: answer.join(', ') })
        } else {
          formattedAnswers.push({ label, value: String(answer) })
        }
      })
      ...
      const response = await fetch('/api/estimate', {
        ...
        body: JSON.stringify({
          ...
          totalPrice,
          thanksMessage: activeFormConfig.thanks_message || 'ご検討ありがとうございます。',
        }),
      })
    ...
    >
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8 relative z-10">
              ...
              <div className="space-y-2 pt-1">
                {activeFormConfig ? (
                  <button
                    ...
                    className="w-full py-3.5 hover:opacity-90 active:scale-[0.98] text-white font-extrabold rounded-2xl transition-all shadow-lg shadow-pink-200/50 text-xs cursor-pointer flex items-center justify-center gap-2"
                  >
                    <span>🧮</span> 簡単見積もり・仕様書作成
                  </button>
                ) : (
                  <div className="w-full py-3 px-3 bg-slate-100/80 text-slate-400 font-bold rounded-2xl text-xs text-center border border-slate-200/60">
                    未作成のため制作することができません
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full py-3 bg-white hover:bg-slate-50 text-slate-800 font-extrabold rounded-2xl border border-slate-200 transition-all text-xs cursor-pointer flex items-center justify-center gap-2 shadow-2xs"
                >
        ...
        <section className="bg-white/75 backdrop-blur-xl p-6 sm:p-7 rounded-3xl shadow-xl border border-white/80 space-y-5">
          <h2 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
            <span className="p-1.5 bg-white rounded-lg text-xs shadow-2xs">⚙️</span>{' '}
            制作・受付条件
          </h2>
      ...
      {isEstimateOpen && activeFormConfig && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] sm:rounded-[40px] p-5 sm:p-7 w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl border-4 border-slate-800 relative">
            
            {/* ヘッダー情報 */}
            <div className="flex justify-between items-start pb-4 border-b-2 border-slate-100 shrink-0">
              <div>
                <span
                  className={`text-[10px] font-black px-3 py-1 rounded-full text-white ${
                    activeFormConfig.is_accepting !== false ? '' : 'bg-slate-400'
                  }`}
                  style={{
                    backgroundColor:
                      activeFormConfig.is_accepting !== false ? themeColor : undefined,
                  }}
                >
                  {activeFormConfig.is_accepting !== false ? '受付中' : '受付停止中'}
                </span>
            ...
            <div className="overflow-y-auto py-5 space-y-5 flex-1 pr-1">
              {!generatedSpec ? (
                <>
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full text-xs p-3 font-bold rounded-2xl border-2 border-slate-100 bg-slate-50 focus:outline-none focus:border-slate-300"
                    />
                  </div>

                  {activeFormConfig.fields.map((f) => {
                    // 📌 クリエイターからの注意事項・説明
                    if (f.type === 'note') {
                      return (
                        <div
                    ...
                    if (f.type === 'faq') {
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs font-bold text-slate-600 pl-3 border-l-2 border-sky-300 whitespace-pre-wrap">
                            {f.faqAnswer}
                          </div>
                        </div>
                      )
                      ...
                      <div key={f.id} className="space-y-2">
                        ...
                        {f.type === 'text' && (
                          <input
                            ...
                            className="w-full text-xs p-3 font-bold rounded-2xl border-2 border-slate-100 bg-slate-50 focus:outline-none"
                          />
                        )}
                        ...
                        {f.type === 'color' && (
                          <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-2xl border-2 border-slate-100">
                            <input
                              type="color"
                              value={formAnswers[f.id] || '#ffffff'}
                              onChange={(e) => handleInputChange(f.id, e.target.value)}
                              className="h-9 w-16 rounded-xl border border-slate-200 cursor-pointer"
                            />
                            <span className="text-xs font-bold text-slate-500 font-mono">
                              {formAnswers[f.id] || '#ffffff'}
                            </span>
                          </div>
                        )}
                        ...
                        {(f.type === 'radio' || f.type === 'checkbox') && (
                          <div className="space-y-1.5">
                            {f.options?.map((opt, oIdx) => {
                              const isChecked =
                                f.type === 'radio'
                                  ? formAnswers[f.id] === opt.label
                                  : (formAnswers[f.id] || []).includes(opt.label)

                              return (
                                <label
                                  key={oIdx}
                                  onClick={() =>
                                    handleInputChange(f.id, opt.label, f.type === 'checkbox')
                                  }
                                  className={`flex items-center justify-between text-xs font-bold px-3.5 py-2.5 rounded-2xl border-2 cursor-pointer transition ${
                                    isChecked
                                  ...
                                  {opt.price > 0 && (
                                    <span
                                      className={`text-[10px] font-black ${
                                        isChecked ? 'text-pink-300' : ''
                                      }`}
                                      style={{ color: !isChecked ? themeColor : undefined }}
                                    >
                ...
                /* ✨ 送信完了・仕様書出力画面（テキストコピー＆PDF出力） */
                <div className="space-y-5 animate-in fade-in">
                  {/* 生成された仕様書テキスト */}
                  <div className="bg-slate-900 text-slate-100 p-4 rounded-2xl font-mono text-xs leading-relaxed whitespace-pre-wrap select-all border border-slate-800 shadow-inner max-h-56 overflow-y-auto">
                    {generatedSpec}
                  </div>

                  {/* 出力アクション（コピー & PDFダウンロード） */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    <button
                      onClick={handleCopySpec}
                      className="py-3.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold rounded-2xl transition text-xs flex items-center justify-center gap-2 cursor-pointer shadow-md active:scale-95"
                    >
                      <span className="text-sm">{copied ? '✅' : '📋'}</span>
                      <span>{copied ? 'コピーしました！' : '仕様書テキストをコピー'}</span>
                    </button>

                    <button
                      onClick={handleDownloadPDF}
                      disabled={isDownloadingPdf}
                      className="py-3.5 px-4 bg-rose-600 hover:bg-rose-700 disabled:bg-rose-300 text-white font-extrabold rounded-2xl transition text-xs flex items-center justify-center gap-2 cursor-pointer shadow-md active:scale-95"
                    >
                      <span className="text-sm">📄</span>
                      <span>
                        {isDownloadingPdf ? 'PDFを生成中...' : 'PDF形式でダウンロード'}
                      </span>
                    </button>
                  </div>
                    ...
                    <div className="space-y-2">
                      {profile.twitter_url && (
                        <a
                          ...
                          className="w-full py-3 px-4 bg-slate-900 hover:bg-slate-800 text-white font-extrabold rounded-2xl transition flex items-center justify-between text-xs shadow-xs"
                        >
                          <span>X (Twitter) の DM で送信する</span>
                      ...
                      {profile.external_estimation_url && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-full py-3 px-4 text-white font-extrabold rounded-2xl transition flex items-center justify-between text-xs shadow-xs"
                          style={{ backgroundColor: themeColor }}
                        >
                          <span>外部フォーム / Webサイトで送信する</span>
                          <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-md">
                            開く ↗
                          </span>
                        </a>
            ...
            {/* フッター：金額 ＆ 仕様書作成ボタン（未作成時のみ表示） */}
            {!generatedSpec && (
              <div
                className="pt-4 border-t-2 border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0 p-4 rounded-2xl"
              ...
              >
                ...
                <button
                  ...
                  style={{ backgroundColor: themeColor }}
                  className="w-full sm:w-auto py-3 px-6 hover:opacity-90 active:scale-95 text-white font-extrabold rounded-2xl transition text-xs shadow-lg cursor-pointer"
                >
                  この内容で仕様書を作成 ➔
                </button>
              </div>
      ...
      {isContactOpen && (
        <div className="fixed inset-0 bg-slate-950/50 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 sm:p-7 w-full max-w-md shadow-2xl border border-white relative space-y-6">
            <button
